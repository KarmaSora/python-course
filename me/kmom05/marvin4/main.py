...
import emission_functions as emfn
...
def main():
    ...
    while not stop:
        ...
        print("13) country emmision searcher ")
        print("14) SearchByYear")
        print("15) enter countryname and get dict ")
        ...
        if choice == "q":
            ...
            try:
                country_search = input("enter countryData to search: ")
                search_data = emfn.search_country(country_search)
                stringfied_list = ", ".join(search_data)
                print("Following countries were found: "+stringfied_list)
            except ValueError as error: 
                print(str(error))

        elif choice == "14":
            try:
                data= input("Enter country and one year, or two: ")
                """ Sweden,1990"""
                """ Sweden,1990,2017"""

                ans = emfn.co2emmision(data)
                print(ans)
            except ValueError as error:
                print(error)

        elif choice =="15":
            data = input("enter a country name: ")
            to_print = emfn.get_country_data(data)
            emfn.print_country_data(to_print)
